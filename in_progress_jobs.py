    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                description = description_element.inner_text() if description_element else "Description not found"

                if title != "Title not found" and description != "Description not found":
                    return title, description

                print(f"Attempt {attempt + 1}: Title or description not found, retrying...")
    ...
    with open(csv_filename, "r", encoding="utf-8") as file:
        ...
        for row in reader:
            if row["url"] == parent_url:
                row["in_progress_links"] = " ; ".join(in_progress_links) if in_progress_links else ""
            rows.append(row)
...
def update_csv_with_details(parent_url, in_progress_url, title, description, csv_filename):
    ...
    with open(csv_filename, "r", encoding="utf-8") as file:
        ...
        for row in reader:
            if row["url"] == parent_url:
                titles = row["in_progress_titles"].split(" ; ") if row["in_progress_titles"] else []
                descriptions = row["in_progress_descriptions"].split(" ; ") if row["in_progress_descriptions"] else []
                links = row["in_progress_links"].split(" ; ") if row["in_progress_links"] else []

                try:
                    clean_links = [link.replace("https://www.upwork.com", "") for link in links]
                    ...
                    print(f"Link {in_progress_url} not found in parent job {parent_url}")
            rows.append(row)
